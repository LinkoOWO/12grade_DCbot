...
class Manager(commands.Cog):
    ...
    async def load_cog(self, interaction: discord.Interaction, cog: str):
        try:
            ...
            await interaction.response.send_message(f"好耶是大佬裝(((o(*ﾟ▽ﾟ*)o))) `{cog}`")
        except Exception as e:
            await interaction.response.send_message(f"沒機緣了(╥﹏╥) {e}")
    ...
    async def unload_cog(self, interaction: discord.Interaction, cog: str):
        try:
            ...
            await interaction.response.send_message(f"只有新手裝了(╥﹏╥)")
        except Exception as e:
            await interaction.response.send_message(f"想打敗我( ꜆⌯'▾'⌯)꜆ 先練個3000年吧( ｰ̀֊ｰ́ )✧ {e}")
    ...
    async def reload_cog(self, interaction: discord.Interaction, cog: str):
        try:
            ...
            await interaction.response.send_message(f"洗完啦~ Cog: `{cog}`")
        except Exception as e:
            await interaction.response.send_message(f"洗衣機壞了(╥﹏╥) {e}")
